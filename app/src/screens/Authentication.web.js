...
import { Heading, Text, Tabs, TabItem, Flex, View, Image, useTheme, Authenticator } from '@aws-amplify/ui-react';
...
import { UserType, UserTypeList, reconfigureAmplifyForUserType } from "../userTypes";
...
export default function withAuthenticator(Component) {
    const AppWithAuthenticator: FunctionComponent<T> = props => {
        ...
        useEffect(async () => {
            if(accountType !== null) {
                await AsyncStorage.setItem('@last_account_selection', UserTypeList[accountType])
