...
import {
  ...
  SealedRecordDynamoLatestToUpdateType,
  SealedRecordDynamoUpdateType,
  RecordMetadata,
  ...
  sealedRecordSchemaDynamoLatestToUpdate,
...
export const handler: APIGatewayProxyWithCognitoAuthorizerHandler = async (
  ...
  try {
    ...
    // Sealed records can only be updated by adding associated records to them.
    if (existingRecord.sealed) {
      ...
      try {
        ...
        return bad([e], 'Unknown error sealed')
      }
    } else {
      ...
      try {
        ...
        return bad(
          e,
          'You did not upload a new manifest, or uploaded a bad one'
        )
      }
      {
        if (newManifest.root) {
      ...
      try {
        ...
        return bad([e], 'Unknown error committing to s3')
      }
    }
  } catch (e) {
